...
export class ProductsService {
  ...
  public getDepartments(){
    ...
    for(let i  = 0; i < this.products.length; i++){
      if(departments.length == 0){
        departments.push(this.products[i]['department']);
      }else if(!departments.includes(this.products[i]['department'])){
        departments.push(this.products[i]['department']);
      }
    }
  ...
  public getSamples(){
    ...
    for(let i = 0; i < this.getDepartments().length ; i++){
      samples[i] = [];
      for(let j = 0; j < 3; j++){
        samples[i][j] = new Object();
        let ran = Math.floor((Math.random()*this.products.length));

        if(this.products[ran]['department'] == this.getDepartments()[i] && !samples[i].includes(this.products[ran]) ){
          samples[i][j] = this.products[ran];
        }else{
          j--;
        }
      }
